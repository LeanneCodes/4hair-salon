...
def weekly_data_input():
    ...
    while True:
        ...
        print("Enter your weekly sales figures in the order below:")
...
def validate_data(value1, value2):
    ...
    try:
        ...
        if len(value1) != 6 or len(value2) != 6:
            raise ValueError(
                "Exactly 6 values are required for both separated by commas." +
...
def abv_reccos(data):
    ...
    """
    print("Recommendations to improve ABV per city are below, if any...\n")
    ...
    for key, value in abv_dict.items():
        ...
        if abv_value[key] < (0.1 * value[1]):
            print("This location is severely underperforming.\nTry" +
                  ...
                  " Balayage/Foilayage\n5. Blowout\n6. Bridal Hair\n7." +
                  " 4Hair's Luxury Hair Care Kits\n")
        elif abv_value[key] < (0.3 * value[1]):
            print("Your average booking value for this location is " +
                  "dangerously low.\nTry upselling your clients with" +
                  " services that are in the £25-£40 range.\n")
        elif abv_value[key] < (0.5 * value[1]):
            print("This location is more than 50% under target.\nEncourage" +
                  " clients to trial the latest hairstyle trends that" +
                  " are £30+.\n")
        elif abv_value[key] < (0.7 * value[1]):
            print("This city's salon is currently falling behind.\nAim to" +
                  " sell 4Hair's luxury hair care kits to each client to " +
                  "exceed targets.\n")
        elif abv_value[key] < (0.95 * value[1]):
            print("So close to target, but not quite there yet.\nEncourage" +
                  " clients to buy the hair oil serums and masks after " +
                  "every appointment.\n")
        elif abv_value[key] <= (0.99 * value[1]):
            print("This salon is doing really well.\nIncentivise " +
                  "employees with 45% off £50+ treatments for themselves or" +
                  " family, only if they exceed target next week.\n")
        elif abv_value[key] > value[1]:
            print("Congratulations! This city is smashing target goals." +
                  " Keep it up!\n")
        else:
            print("This salon is meeting target.\nHowever, aim to" +
                  " increase the salon's ABV to prevent falling behind.\n")

    print("To get access to the 4Hair Salon Google worksheet, please copy " +
